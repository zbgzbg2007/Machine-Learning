...
class Net(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        ...
        self.actor_log_var = nn.Parameter(torch.zeros(1, num_outputs) -1.6)
...
def train(rank, args, shared_model, T):
    ...
    while True:
        ...
        for step in range(args.num_steps):
            ...
            obs, reward, done, _ = env.step(to_numpy(action, args.gpu)[0])
        ...
        for i in reversed(range(len(rewards))):
            ...
            #gae = gae * gamma * args.lambd + delta
 
            #value_loss = value_loss + 0.5 * gae.pow(2)
            policy_loss = policy_loss - \
                log_probs[i] * Variable(advantage.data) 

        optimizer.zero_grad()

        (policy_loss.mean() + value_loss).backward()
...
parser.add_argument('--num-processes', type=int, default=6, metavar='NP', help='number of processes to use')
parser.add_argument('--num-steps', type=int, default=50, metavar='NS', help='number of forward steps')
...
parser.add_argument('--lambd', type=float, default=0.95, metavar='LMD')
