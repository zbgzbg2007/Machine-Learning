...
class Agent:
    ...
    def update_network(self):
        ...
        if self.double:
            ...
            outputs = rewards + next_y[range(next_y.shape[0]), target_actions]
        else:
            outputs = rewards + np.max(next_y, axis=1)
    ...
    def fit(self, env, num_iterations, max_episode_length=5000):
        ...
        history = np.empty((self.window, 80, 80), dtype='uint8') # history stores the last self.window number of  processed frames as input for network
        for episode in range(num_iterations):
            ...
            for i in range(self.window):
                ...
                state = self.process_state_for_network(obs, last_obs)
                history[i] = state
            index = 0 # index for history 
            for t in range(max_episode_length):
                #env.render()
                state = history.astype('float32') / 255.
    ...
    def evaluate(self, env, num_episodes, max_episode_length=5000):
        ...
        history = np.empty((self.window, 80, 80), dtype='uint8')
