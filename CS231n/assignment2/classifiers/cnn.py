...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    ############################################################################
    C, H, W = input_dim
    # Here we assume filter_size is always odd number
    # The output of convolutional layer should have the same shape as its input
    

    self.params['W1'] = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size)
    self.params['b1'] = np.zeros(num_filters)
    self.params['W2'] = weight_scale * np.random.randn(H * W * num_filters / 4, hidden_dim)
    # there is a pooling layer, so the size of W2 is reduced by 4
